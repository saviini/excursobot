...
• Убедись, что локация корректна`;
  ...
  // Метод для запуска в режиме polling
  async startPolling(): Promise<void> {
    Logger.info('bot_startup', 'Запуск бота в режиме polling');

    try {
      // Сначала пытаемся удалить webhook, если он был установлен
      try {
        await this.bot.api.deleteWebhook();
        Logger.info('bot_startup', 'Webhook успешно удален');
      } catch (error) {
        // Игнорируем ошибку 404 - это означает, что webhook не был установлен
  ...
  // Метод для запуска webhook
  async startWebhook(port: number, webhookUrl: string): Promise<void> {
    Logger.info('bot_startup', 'Запуск бота в режиме webhook', {
      port,
      webhookUrl,
    });
    ...
    try {
      ...
      // Здесь можно добавить HTTP сервер для webhook
      // Пока что используем polling для простоты
